...
export type Invoice = {
  ...
  items: {
    ...
    costPrice?: number; // FIFO cost from batch at sale time
  }[];
  subtotal: number;
  ...
  balanceDue: number;
  changeGiven?: number;
  // Included relations (when fetched with includes)
  customer?: Customer;
  vehicle?: Vehicle;
  employee?: Employee;
};
...
export const recentInvoicesData: (Omit<Invoice, 'invoiceNumber' | 'customerId' | 'vehicleId' | 'employeeId' | 'items' | 'subtotal' | 'globalDiscountPercent' | 'globalDiscountAmount' | 'balanceDue' | 'payments' | 'customer' | 'vehicle' | 'employee'> & { customerName: string, amount: number, status: 'Paid' | 'Pending' | 'Overdue' })[] = [
  {
    id: "INV-2024005",
