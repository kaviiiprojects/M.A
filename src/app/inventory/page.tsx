...
export default function InventoryPage() {
  ...
  const fetchData = useCallback(async (signal: AbortSignal) => {
    try {
      ...
      const [productsRes, servicesRes] = await Promise.all([
        fetch('/api/products?limit=1000', { signal }), // Fetch decent amount for inventory view
      ...
      const validatedProducts = ProductsSchema.safeParse(productsData.products || []);
  ...
  const handleAddStock = useCallback(async (productId: string, quantity: number, costPrice: number): Promise<void> => {
    try {
      // Use the new inventory batch API for FIFO tracking
      const res = await fetch("/api/inventory-batches", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ productId, quantity, costPrice }),
      });
      ...
      toast({ title: "Stock Added", description: `Added ${quantity} units at Rs. ${costPrice.toFixed(2)} per unit.` });
      
      // Refresh products to get updated stock count
      const refreshController = new AbortController();
      fetchData(refreshController.signal);
