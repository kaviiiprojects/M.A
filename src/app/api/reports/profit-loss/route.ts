...
interface ExtendedInvoiceItem {
  ...
  unitPrice: number;
  total: number;
  costPrice?: number; // Actual cost from FIFO batch at sale time
  buyPrice?: number; // Legacy field for backwards compatibility
  discount: number;
}
  ...
  try {
    ...
    for (const invoice of processedInvoices) {
      ...
      for (const item of invoice.items) {
        ...
        // Use costPrice from FIFO batch (primary), fallback to buyPrice (legacy), then product.actualPrice
        let costPerUnit = item.costPrice ?? item.buyPrice ?? product.actualPrice ?? 0;
        const isEstimate = item.costPrice === undefined && item.buyPrice === undefined;
