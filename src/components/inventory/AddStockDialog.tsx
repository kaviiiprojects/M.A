...
type AddStockDialogProps = {
  ...
  onAddStock: (productId: string, quantity: number, costPrice: number) => Promise<void>;
  isOpen: boolean;
...
export function AddStockDialog({ 
  ...
  // Set default cost price when product is selected
  useEffect(() => {
    if (selectedProduct) {
      setCostPrice(selectedProduct.actualPrice);
    }
  }, [selectedProduct]);
  ...
  const handleUpdateStock = async () => {
    if (!selectedProduct || quantity <= 0 || !costPrice || costPrice <= 0) return;
    
    setIsSubmitting(true);
    try {
      await onAddStock(selectedProduct.id, quantity, costPrice);
      ...
      <DialogContent className="sm:max-w-xl rounded-none border-zinc-200">
        <DialogHeader>
          ...
          <DialogDescription className="text-zinc-500">
            Select a product and enter the quantity and cost price for this batch.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex gap-6 py-4">
            ...
            {/* Right side: Quantity and Cost Input */}
            <div className="w-1/2 flex flex-col justify-center items-center bg-zinc-50 p-6 rounded-sm">
                {selectedProduct ? (
                    <div className="w-full space-y-4 text-center">
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="costPrice" className="text-xs uppercase tracking-widest text-zinc-500">
                            ...
                            <Input
                                ...
                                className={cn(commonInputStyles, "text-center text-xl h-12")}
                                placeholder="Enter cost price"
                            />
                            <p className="text-xs text-zinc-400">
                                Default: Rs. {selectedProduct.actualPrice.toFixed(2)}
                            </p>
                        </div>
                    </div>
                ) : (
        ...
        <DialogFooter className="gap-2">
          ...
          <Button
            ...
            disabled={!selectedProduct || quantity <= 0 || !costPrice || costPrice <= 0 || isSubmitting}
