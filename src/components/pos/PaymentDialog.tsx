...
export function PaymentDialog({
  ...
  const { totalPaid, remainingBalance, changeGiven, paymentStatus, isChequeInfoMissing } = useMemo(() => {
  ...
  const handleConfirm = () => {
    ...
    // Filter out any payments with zero or negative amount
  ...
  const isConfirmDisabled = isProcessing || isChequeInfoMissing;
            ...
            return (
              <div key={payment.id} className="grid grid-cols-12 gap-4 items-start mb-4">
                ...
                <div className="col-span-3 space-y-1.5">
                  ...
                  <div className="flex flex-col items-center gap-1 rounded-md bg-zinc-100 p-1">
                    {availableMethods.map((m) => (
                      <button
                        key={m.id}
                        onClick={() => handleUpdatePayment(payment.id!, { method: m.id as PaymentMethod })}
                        className={cn(
                          "flex-1 flex items-center justify-center gap-2 p-2 rounded-sm transition-all duration-200 text-zinc-400",
                      ...
                      >
                        ...
                        <span className="text-xs font-medium uppercase tracking-wider">{m.id}</span>
                      </button>
                    ))}
                  </div>
                </div>
