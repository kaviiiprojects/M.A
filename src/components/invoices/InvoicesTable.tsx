...
const formatDate = (dateValue: number | string | Date | any) => {
    ...
    try {
        ...
        if (dateValue instanceof Date) {
             ...
             date = toDate(dateValue, { timeZone: 'Asia/Colombo' });
        } else if (typeof dateValue === 'string') {
             date = new Date(dateValue);
        } else if (typeof dateValue === 'object' && 'seconds' in dateValue) {
             // Handle Firestore Timestamp like object if it ever comes up (seconds * 1000)
        ...
        <TableRow key={invoice.id} className="border-zinc-100 text-sm hover:bg-zinc-50/50">
            ...
            <TableCell className="p-2 h-12">
                <div className="flex flex-col justify-center">
                    ...
                    <span className="text-[10px] text-zinc-500">{invoice.customerDetails?.phone || 'N/A'}</span>
                </div>
            </TableCell>
            <TableCell className="p-2 h-12">{invoice.vehicleModel || invoice.vehicleDetails?.model || 'N/A'}</TableCell>
